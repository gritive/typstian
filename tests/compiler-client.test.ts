...
describe("TypstianCompilerClient", () => {
  it("loads one WASM engine lazily and checks the environment", async () => {
    ...
    engines[0]!.respond({ type: "environment", protocolVersion: 1, typstVersion: "0.15.1" });

    await expect(result).resolves.toEqual({ protocolVersion: 1, typstVersion: "0.15.1" });
...
it("passes the pinned overlay snapshot to the engine compile request", async () => {
  ...
  it("disposes an aborted compile session and can load a fresh engine", async () => {
    ...
    await vi.waitFor(() => expect(engines[1]?.calls).toHaveLength(1));
    engines[1]!.respond({ type: "environment", protocolVersion: 1, typstVersion: "0.15.1" });
    await expect(environment).resolves.toEqual({ protocolVersion: 1, typstVersion: "0.15.1" });
    client.close();
  });
